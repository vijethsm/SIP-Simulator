...
A single-file, self-contained protocol analyzer that visualizes SIP (Session Initiation Protocol) call flows as animated sequence diagrams — with plain-English explanations running alongside the raw packets, so it's useful both to someone learning SIP for the first time and to someone debugging a live PBX/trunk issue.

No build step, no server, no dependencies. Open `index.html` in a browser and it runs.
...
Everything lives in one HTML file: inline `<style>`, then a single `<script>` block. There is no build step and no external JS dependencies.
